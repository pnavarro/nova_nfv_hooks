...
from nova.virt.libvirt import host as virthost
...
class LibvirtDriver(driver.ComputeDriver):
        ...
        if not guest_cpu_numa_config:
            ...
            if topology:
                ...
                if len(pci_cells) == 0:
                    ...
                    if vcpus <= len(cell.cpuset) and memory <= cell.memory:
                        return cell.cpuset, None, None, None
    ...
    def _get_guest_config(self, instance, network_info, image_meta,
        ...
        cpuset, cputune, guest_cpu_numa, guest_numa_tune = \
            self._get_guest_numa_config(
                instance_numa_topology, flavor, pci_devs, allowed_cpus)
        ...
        guest.membacking = self._get_guest_memory_backing_config(
            instance_numa_topology, guest_numa_tune)
    ...
    def _live_migration_monitor(self, context, instance, dest, post_method,
        ...
        n = 0
        while True:
            info = virthost.DomainJobInfo.for_domain(dom)
    ...
    def pre_live_migration(self, context, instance, block_device_info,
        ...
        if not (is_shared_instance_path or is_shared_block_storage):
            ...
            # NOTE(pbrady): vfat is not read-only so works fine
            if CONF.config_drive_format not in ('vfat',) \
               and configdrive.required_by(instance):
